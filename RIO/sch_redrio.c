                               ...
                               unsigned int backlog,int order)
{
	...
	{
	k=(p->Wlog);
    	return  v->qavg[order] + (backlog - ((v->qavg[order]) >> (k)));
	}
	else
	{
	k=(p->Wlog);
...
static int red_enqueue(struct sk_buff *skb, struct Qdisc *sch)
{
	...
	if(order==1)
	{
		result_qavg=q->vars.qavg[1]+q->vars.qavg[0];
	}
	else
	{
		result_qavg=q->vars.qavg[0];
	}
    
   printk("\n%lu %u %lu %u", q->vars.qavg[0], q->stats.droppedpriority, q->vars.qavg[1], q->stats.droppednormal);
    ...
    switch (red_action(&q->parms, &q->vars, result_qavg,order)) {
    case RED_DONT_MARK:
        break;
...
static struct Qdisc_ops redrio_qdisc_ops __read_mostly = {
    .id        =    "redrio",
...
static int __init redrio_module_init(void)
{
    return register_qdisc(&redrio_qdisc_ops);
}

static void __exit redrio_module_exit(void)
{
    unregister_qdisc(&redrio_qdisc_ops);
}

module_init(redrio_module_init)
module_exit(redrio_module_exit)
